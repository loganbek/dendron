...
export interface INoteStore<K> {
  ...
  /**
   * Get NoteProps non-metadata by fname
   * If fname is not found, return error.
   *
   * @param fname: fname of NoteProps
   * @return NoteProps non-metadata
   */
  getNonMetadata(fname: string): Promise<RespV3<string>>;
   ...
   *
   * @param key: key of NoteProps to write
   * @param note: NoteProps to write
   * @return original key
   */
  write(key: K, note: NoteProps): Promise<RespV3<K>>;
   ...
   *
   * @param key: key of NoteProps to write
   * @param note: NoteProps metadata to write
   * @return original key
   */
  writeMetadata(key: K, note: NoteProps): Promise<RespV3<K>>;
