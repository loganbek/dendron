    ...
    await runEngineTestV5(
      async ({ vaults, wsRoot, engine }) => {
        ...
        _.values(engine.notes).forEach(async (note) => {
          await noteStore.writeMetadata(note.id, note);
        });
        ...
        expect(noteMetadata.body).toBeFalsy();
  ...
  test("Test creating and retrieving note", async () => {
    await runEngineTestV5(
      async ({ vaults, wsRoot, engine }) => {
        ...
        await noteStore.write(newNote.id, newNote);
        ...
        expect(noteMetadata.data!.contentHash).toBeFalsy();
      },
      {
        expect,
      }
  ...
  test("Test multiple writes", async () => {
    await runEngineTestV5(
      async ({ vaults, wsRoot, engine }) => {
        ...
        let writeResp = await noteStore.write(newNote.id, newNote);
        ...
        // Write same note
        writeResp = await noteStore.write(newNote.id, newNote);
        expect(writeResp.data).toBeTruthy();
        const note2 = await noteStore.get(newNote.id);
  ...
  test("Test writing multiple notes with same fname", async () => {
    await runEngineTestV5(
      async ({ vaults, wsRoot, engine }) => {
        ...
        writeResp = await noteStore.write(diffNote.id, diffNote);
        expect(writeResp.data).toBeTruthy();
  ...
  test("Test writing note with mismatched key", async () => {
    await runEngineTestV5(
      async ({ vaults, wsRoot, engine }) => {
        ...
        const writeResp = await noteStore.write("bar", newNote);
  ...
  test("Test multiple write metadata", async () => {
    await runEngineTestV5(
      async ({ vaults, wsRoot, engine }) => {
        ...
        let writeResp = await noteStore.writeMetadata(newNote.id, newNote);
        expect(writeResp.data).toBeTruthy();
        ...
        // Write same note
        writeResp = await noteStore.writeMetadata(newNote.id, newNote);
        expect(writeResp.data).toBeTruthy();
